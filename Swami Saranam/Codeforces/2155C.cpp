...
bool check(int n,const vector<int>a,int startCape){
    vector<int>c(n);
    c[0]=startCape;
    for(int i=0;i<n-1;i++){
        int diff=(a[i+1]-a[i]+MOD)%MOD;
        int nextCape=(1-c[i]-diff+MOD)%MOD;
        if(nextCape!=0&&nextCape!=1)return false;
        c[i+1]=nextCape;
    }
    int tr=0;
    for(int v:c){
    ...
    int a0=((c[0]==0?1:0)+tr)%MOD;
    return (a0 %MOD)==(a[0]%MOD);
}
...
void solve(){
    ...
    if(n==1){
        if(a[0]==1)cout<<2%MOD<<endl;
        else cout<<0<<endl;
        return;
    }
    int sol=0;
    if(check(n,a,0))sol=(sol+1)%MOD;
    if(check(n,a,1))sol=(sol+1)%MOD;
    cout<<sol%MOD<<endl;
}
signed main() {
    fast();
