...
void solve(){
    ...
    vector<pair<int,int>>rw,no_rw;
    vector<int>b(m),c(m);
    for(int i=0;i<m;++i)cin>>b[i];
    for(int i=0;i<m;++i)cin>>c[i];
    for(int i=0;i<m;++i){
        if(c[i]>0)rw.push_back({b[i],-c[i]});
        else no_rw.push_back({b[i],0});
    }
    sort(rw.begin(),rw.end());
    sort(no_rw.begin(),no_rw.end());
    int ans=0;
    ...
    vector<pair<int,int>>fw=no_rw;
    fw.insert(fw.end(),rem.begin(),rem.end());
