...
int badCnt(vector<int>& p){
    int c=0;
    for(int i=0;i+2<sz(p);i++){
        int a=p[i],b=p[i+1],d=p[i+2];
        if(gcd(a,b)==1&&gcd(a,d)==1&&gcd(b,d)==1) c++;
    }
    return c;
}
...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        if(i%3==0) m3.pb(i);
        else if(i%2==0) m2.pb(i);
        else u.pb(i);
    }
    s=m3;
    s.insert(s.end(),all(m2));
    vector<int> p(n);
    int si=0;
    int ui=0;
    while(si<sz(s)||ui<sz(u)){
        if(si<sz(s)) p.pb(s[si++]);
        if(si<sz(s)) p.pb(s[si++]);
        if(ui<sz(u)) p.pb(u[ui++]);
    }
