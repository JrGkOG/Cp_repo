...
//     for(int i=0;i<n;i++){
//     cin>>left[i]>>right[i];
//     initial_sum+=(right[i]-left[i]);
// }
    ll ans=base;
    if((n+1)/2>=2){
        ll f=1;
        ll l=(n%2!=0?n:n-1);
        ans=max(ans,base+(l-f));
    }
    ...
    for(int i=0;i<n;++i){
        ll pos=i+1;
        if(pos%2!=0)minOdd=min(minOdd,2*a[i]+pos);
        else{
            if(minOdd!=INT_MAX){
                ll gain=(2*a[i]+pos)-minOdd;
    ...
    for(int i=0;i<n;++i){
        ll pos=i+1;
        if(pos%2==0)maxEven=max(maxEven,2*a[i]-pos);
        else{
            if(maxEven!=INT_MIN){
                ll gain=maxEven-(2*a[i]-pos);
                ans=max(ans,base+gain);
            }
        }
    }
    // ll lastOdd=(n%2==1 ? n : n-1);
    // if(lastOdd>=1)ans=max(ans,base+(lastOdd-1));
 
    // ll lastEven=(n%2==0 ? n : n-1);
    // if(lastEven>=2)ans=max(ans,base+(lastEven-2));
    cout<<ans<<endl;
}
signed main() {
    fast();
