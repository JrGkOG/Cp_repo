...
ll get_prefix(ll i, ll k) {
    return i * (2 * k + i - 1) / 2;
}
ll prefix_sum(ll i, ll k) {
    return i * (2 * k + i - 1) / 2;
}
ll func(ll i, ll n, ll k, ll total) {
    ll pre = prefix_sum(i, k);
    return abs(2 * pre - total);
}
void solve(){
    ll n, k; cin >> n >> k;
    ...
    while (low < high) {
        ll mid = (low + high) / 2;
        ll val_mid = func(mid, n, k, total);
        ll val_next = func(mid + 1, n, k, total);
