...
void solve() {
    ...
    for (int k = 1; k <= n; k++) {
        ...
        for (int i = 1; i <= 2 * n; i++) {
            ...
            int currentTime = a[i - 1];
            for (int j = 0; j <= k; j++) {
                int enterVal = -INF;
                int exitVal = -INF;
                if (j > 0 && dp1[j - 1] != -INF) enterVal = dp1[j - 1] - currentTime;
                if (j + 1 <= k && dp1[j + 1] != -INF) exitVal = dp1[j + 1] + currentTime;
                dp2[j] = max(enterVal, exitVal);
            }
            dp1 = dp2;
        }
        answers.push_back(dp1[0]);
    }
    for(auto it:answers){
        cout<<it<<" ";
    }
