...
ll mergeCnt(vector<int>&a,vector<int>&t,int l,int m,int r){
    ll inv=0;
    int i=l,j=m+1,k=l;
    while(i<=m&&j<=r){
        if(a[i]<=a[j]) t[k++]=a[i++];
...
ll msort(vector<int>&a,vector<int>&t,int l,int r){
    if(l>=r) return 0;
    int m=(l+r)/2;
    ll inv=0;
    inv+=msort(a,t,l,m);
    inv+=msort(a,t,m+1,r);
    inv+=mergeCnt(a,t,l,m,r);
    return inv;
}
...
void solve(){
    ...
    vector<int>a;
    for(int i=1;i<=n;i++) a.push_back(pos[i]);
    vector<int>t(n);
    cout<<msort(a,t,0,n-1)<<endl;
}
signed main() {
    fast();
    ll t = 1;
